...
sub run {
    ...
    my $context = Diary::Context->from_env($env);
    my $route = Diary::Config->router->match($env) or $context->throw(404);
    $context->req->path_parameters(%{$route->{parameters}});

    my $destination = $route->{destination} or $context->throw(404);
    $destination->{engine} or $context->throw(404);
    ...
    my $handler = $engine->can($action) or $context->throw(501);
