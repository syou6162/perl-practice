...
sub _build_request {
    ...
    return Diary::Request->new($self->env);
};
...
sub render_file {
    ...
    require Diary::View::Xslate;
    my $content = Diary::View::Xslate->render_file($file, {
        c => $self,
        %{ $self->stash },
        %$args
    });
