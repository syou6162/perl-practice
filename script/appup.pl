...
my $config = 'Diary::Config';
eval "require $config" or die $@;

my $runner = Plack::Runner::Diary->new;
...
$ENV{INTERN_BOOKMARK_ENV} = $runner->{env} || 'local';
$runner->{env} = 'development';

my $options = +{ @{ $runner->{options} } };

my $server = Proclet::Diary->new(
    color => 1,
    log => {
...
if ($options->{kyt_prof}) {
    ...
    Devel::KYTProf->namespace_regex(qr/^Diary/);
}

$server->run;

package Plack::Runner::Diary;
use strict;
use warnings;
